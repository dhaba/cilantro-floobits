...
class ReactorCore(Thread):
    ...
    def __init__(self, queue):
        ...
        self.ctx, self.socket, self.future = None, None, None
    ...
    def execute(self, cmd: Command):
        ...
        if cmd.type == Command.SUB:
            ...
            self.socket.connect(cmd.kwargs['url'])

            future = asyncio.ensure_future(self.receive(cmd.kwargs['callback']))
            ...
            if not self.pub_socket:
                ...
                self.pub_socket.bind(cmd.kwargs['url'])
            ...
            self.pub_socket.send(cmd.kwargs['data'])

        elif cmd.type == Command.UNSUB:
            self.log.debug("Canceling futures...")
            self.future.cancel()
